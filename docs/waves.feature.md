...
- **2026-07-02** - volledige repo-audit (zie `AUDIT.md`): hero-spectrum-bug gefixt (CSS won van stroke-attribuut), docs-drift gecorrigeerd (periodes, harsh-tag 16, 3 ontbrekende closing-tags, nav-404's, amplitude-claim), site laadt lib nu **lokaal** i.p.v. CDN-gepind. Engine: reduced-motion live gevolgd + optionele `destroy(state, node)`-hook. Build reproduceerbaar: `tools/build.js` + `tools/parity.js` (**1539/1539** bit-identiek core/bundle/min, in-repo). Nieuw: `docs/art.html` ("nocturne for 34 waves") + nav-item Nocturne. Math ongewijzigd - canon blijft v3.4.0 / `6ce959e`.
